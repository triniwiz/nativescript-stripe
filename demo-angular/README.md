# NativeScript Stripe - Angular Demo
...
1.  Clone this repo, and from [src](../src) run `npm install`.  This installs dependencies for nativescript-stripe plugin.  **Note**: this step is not necessary when you install `nativescript-stripe` into your project via `nsc plugin add nativescript-stripe`.
...
1.  Run `nsc run [ios|android]`, and test using a [test card number](https://stripe.com/docs/testing)

## Accept Apple Pay (iOS only)

1.  Kill the `nsc run ios` process if running
...
1.  Run `nsc run ios --emulator`.  The iOS emulator should have a test Apple Pay account setup.
...
**Highly recomended:** enable apple pay on an iPhone and run: `nsc run ios`.  Make sure apple pay shows up.  If it does not, check out https://github.com/triniwiz/nativescript-stripe/issues/50
